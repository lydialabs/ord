...
pub(crate) struct MintBumpfee {
  ...
  #[clap(
    long,
    help = "Make sure to have minimum <AMOUNT> postage with mint output. [default: 10000sat]"
  )]
...
impl MintBumpfee {
  pub(crate) fn run(self, wallet: Wallet) -> SubcommandResult {
      ...
      "`ord wallet mint-bumpfee` requires index created with `--index-runes` flag",
    );
    ...
    let destination = self.destination.require_network(chain.network())?;
    ...
    // assert to make sure the change receiver is also the rune destination
    assert!(
      signed_transaction.output[0].script_pubkey == destination.script_pubkey()
    );
    assert!(
      signed_transaction.output[0].value >= postage.to_sat()
    );
